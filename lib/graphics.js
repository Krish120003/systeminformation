...
function graphics(callback) {
  ...
  function parseLinesLinuxControllers(lines, ignoredPciBuses) {
    let controllers = [];
    let currentController = {
      vendor: '',
      model: '',
        ...
        if (' ' !== lines[i][0] && '\t' !== lines[i][0]) {        // first line of new entry
          ...
          if (vgapos !== -1 || _3dcontrollerpos !== -1) {         // VGA
            ...
            if (currentController.vendor || currentController.model || currentController.bus || currentController.vram !== -1 || currentController.vramDynamic) { // already a controller found
              if (!ignoredPciBuses.includes(currentController.pciBus)) { // if not already found by another method
                controllers.push(currentController);
              }
              currentController = {
                vendor: '',
            ...
            currentController.pciBus = lines[i].substr(0, vgapos).trim();
            if (parts.length > 1) {
    ...
    if (currentController.vendor || currentController.model || currentController.bus  || currentController.pciBus || currentController.vram !== -1 || currentController.vramDynamic) { // already a controller found
      if (!ignoredPciBuses.includes(currentController.pciBus)) { // if not already found by another method
        controllers.push(currentController);
      }
    }
    ...
    while ((m = regex.exec(lines))) {
      ...
      switch (m[2]) {
        ...
        case 'CL_DEVICE_TOPOLOGY_AMD':
          controllers[m[1]]['pciBus'] = m[3].match(/[a-zA-Z0-9]+:\d+.\d+/gm)[0];
          devicesFoundPciAddress.push(m[3].match(/[a-zA-Z0-9]+:\d+.\d+/gm)[0]);
          break;
        case 'CL_DEVICE_PCI_BUS_ID_NV':
          pciBusesNvidia[m[1]]['busId'] = m[3];
          break;
        case 'CL_DEVICE_PCI_SLOT_ID_NV':
          pciBusesNvidia[m[1]]['slotId'] = m[3];
          break;
      }
    }
    Object.keys(controllers).forEach(key => {
      let pciInfo = pciBusesNvidia[key];
      if (pciInfo.busId && pciInfo.slotId) {
        let bus = parseInt(pciInfo.busId) & 0xff;
        let device = (parseInt(pciInfo.slotId) >> 3) & 0xff;
        let func = parseInt(pciInfo.slotId) & 7;
        devicesFoundPciAddress.push(`${bus.toString().padStart(2, '0')}:${device.toString().padStart(2, '0')}.${func}`);
        controllers[key]['pciBus'] = `${bus.toString().padStart(2, '0')}:${device.toString().padStart(2, '0')}.${func}`;
      }
    });
    return [Object.values(controllers), devicesFoundPciAddress];
  }
        ...
        if (util.isRaspberry() && util.isRaspbian()) {
          ...
          exec(cmd, function (error, stdout) {
            ...
            exec(cmd, function (error, stdout) {
              ...
              // Controllers
              let controllersAlreadyFound = [];
              let cmd = "clinfo --raw";
              exec(cmd, function (error, stdout) {

                if (!error) {
                  let parsedOutput = parseClinfoLinesLinuxControllers(stdout);
                  result.controllers = result.controllers.concat(parsedOutput[0]);
                  controllersAlreadyFound = controllersAlreadyFound.concat(parsedOutput[1]);
                }
                let cmd = 'lspci -vvv  2>/dev/null';
                exec(cmd, function (error, stdout) {
                  if (!error) {
                    let lines = stdout.toString().split('\n');
                    result.controllers = result.controllers.concat(parseLinesLinuxControllers(lines, controllersAlreadyFound));
                  }
                  if (callback) {
                    callback(result);
                  }
